...
description: >
  ...
  (pr-model.md). Le o diff do git, o .refactor-plan.md, o decision log e a
...
# Refactor PR — Pull Request padronizado

Abre PRs seguindo o modelo padrao da empresa (`pr-model.md`). O PR description
...
Usar conventional commits: `type(scope): summary` (max 80 chars).
...
O titulo segue o padrao: `type(scope): summary` (conventional commit).
Se o PR cobre um card RFIA-XX, incluir o ID: `type(scope): summary [RFIA-XX]`.
...
| Regra | Detalhe |
|-------|---------|
| Titulo | `type(scope): summary` conventional commit, max 80 chars |
